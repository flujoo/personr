...
The theory behind this test divides personality into 5 traits or factors. Below is your score on each factor:

```{r echo=FALSE, fig.width=8.5}
d <- score(responses)
ggplot2::ggplot(d, ggplot2::aes(factor, score, fill = factor)) +
  ggplot2::geom_col() +
...
In the plot below, your score on this factor is compared with other people's scores. The plot displays a distribution of these people's scores on this factor. The percentage on this plot tells the percentage of people whose score is lower than yours.

```{r echo=FALSE}
interpret(d, "Agreeableness")
```
...
```{r echo=FALSE}
interpret(d, "Emotional Stability")
```
...
```{r echo=FALSE}
interpret(d, "Intellect/Imagination")
```
